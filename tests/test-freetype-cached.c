...
framebuffer_blit(void)
{
  int x = 0, y = 200, i, j, k, xx, yy, nchars;
  char  *data = NULL, *c;
  ...
  nchars = strlen(TextStr);

  faces = malloc(sizeof(FT_Face)*(nchars+1));
  ...
  /* 
   * Quick and dirty pre generation glyphs.
  */
  ...
  for (i=0; i<nchars; i++)
    {
      ...
      error = FT_New_Face( FreetypeLibrary, TextFont, 0, &faces[i] );
	...
	printf("Error %d occured, exiting\n", error);
      ...
      error = FT_Set_Char_Size( faces[i], 0, 16*64, 72, 72); 

      glyph_index = FT_Get_Char_Index( faces[i], (int)*c );

      error = FT_Load_Glyph(faces[i], glyph_index, FT_LOAD_DEFAULT);
  ...
  if (Verbose)
    {
      printf("test-freetype: pre generated glyphs in %lli ms\n",
